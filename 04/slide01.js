...
let btnIdx=0;//추가>클릭한 버튼의 인덱스(next, prev)
...
button.click(function(){
  current=current+1;
  btnIdx=$(this).index();//btnIdx=현재클릭한 버튼의 인덱스
  //선택된것만 적용하기
  //1.다 지우고 선택된것만 적용(.on)
  button.removeClass("on");
  $(this).addClass("on");//this는 클릭한 대상
  move()//선택된 버튼의 인덱스 :인자(argument)를 전달한다.(btnIdx가 지역변수리)
...
function timer(){
  setInterval(function(){
   ...
   //move(next);//(이동시키는 함수한테 값을전달해줌)지움 여기서 안함 "버튼 인덱스 번호 기준으로 위치움직일거라"
  },speed)
}
...
function move(){
  if(current==btnIdx)return;//continue 친군데 조건이 true-> 밑에문장을 실행하지않고 돌아감
...
clearAuto();
function clearAuto(){
$("#brandVisual,#buttonList,.controls").mouseenter(function(){clearInterval(id)
})
$("#brandVisual,#buttonList,.controls").mouseleave(function(){timer();
})
}
